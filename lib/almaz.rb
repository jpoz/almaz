...
class Almaz
  ...
  class Capture
    ...
    def call(env)
      ...
      @r.push_tail("almaz::#{Almaz.session_variable}::#{env['rack.session'][Almaz.session_variable]}", "#{Time.now.to_s} #{env['REQUEST_METHOD']} #{env['PATH_INFO']} #{env['QUERY_STRING']}#{CGI.unescape(env['rack.input'].read)}")
